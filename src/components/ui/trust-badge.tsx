...
export function TrustBadge({ type = "verified", className }: TrustBadgeProps) {
    if (type === "verified") {
        return (
            <Badge variant="secondary" className={cn("bg-white text-primary font-semibold flex items-center gap-1 shadow-sm px-2 py-0.5 h-6", className)}>
    ...
    if (type === "bestseller") {
        return (
            <Badge className={cn("bg-primary text-primary-foreground border-none flex items-center gap-1 shadow-sm px-2 py-0.5 h-6", className)}>
